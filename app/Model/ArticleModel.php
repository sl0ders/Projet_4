...
class ArticleModel extends Model
{
    ...
    {
        return $this->query("
            SELECT articles.id, articles.publish,articles.title,articles.content, DATE_FORMAT(articles.date,'%d/%m/%Y à %Hh%imin') AS date_fr,
                   (SELECT count(*) 
                    FROM comments 
                    WHERE comments.article_id = articles.id) AS nb_com,
                    chapters.title as Chapter,  
            ...
            FROM articles 
            LEFT JOIN chapters ON chapter_id = chapters.id
            group by articles.id ORDER BY articles.id
            ");
    }
